...
export ASSUMED_ROLE=${PROJECT}-${BUILD_ENV}
bash terraform.sh -e ${BUILD_ENV} -c common -p aws -a init
bash terraform.sh -e ${BUILD_ENV} -c common -p aws -a verify-plan


if [ ${LAMBDA_INVOKE} = "true" ]; then 
    aws lambda invoke \
    --function-name ${PROJECT}-${BUILD_ENV}-lambda-terraform-changes-detector \
    response.txt
fi
